#
# Copyright (c) 2021 Stefan Lohmaier.
#
# This file is part of TransBrl 
# (see https://transbrl.com).
#
...
from sys import getfilesystemencoding, maxsize, platform, version_info
...
liblouis = None
if platform == 'win32':
    liblouisdll = os.path.dirname(__file__)
    if is_64bit():
        liblouisdll = os.path.join(liblouisdll, 'lib', 'win64')
    else:
        liblouisdll = os.path.join(liblouisdll, 'lib', 'win32')
    liblouis = windll.LoadLibrary(os.path.join(liblouisdll, 'liblouis.dll'))
elif platform == 'darwin' and is_64bit():
    liblouisdylib = os.path.dirname(__file__)
    macarch = 'mac64'
    if subprocess.check_output(['sysctl','-n','machdep.cpu.brand_string']).decode('utf-8').startswith('Apple'):
        macarch = 'macarm'
    liblouisdylib = os.path.join(liblouisdylib, 'lib', macarch, 'liblouis.dylib')
    liblouis = cdll.LoadLibrary(liblouisdylib)
else:
...
#set data pth
liblouis.lou_setDataPath(os.path.dirname(os.path.dirname(os.path.abspath(__file__)).encode(fileSystemEncoding)))
